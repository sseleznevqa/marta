...
document.onclick = function(e) {document.marta_magic_click(e)};
...
document.marta_touch = function(element) {
  ...
  for (var i=0, max=everything.length; i < max; i++) {
    ...
    if (check == control) {
      var rect = everything[i].getBoundingClientRect();
      document.marta_test_xx = rect.left+1;
      document.marta_test_yy = rect.top+1;
      document.elementFromPoint(document.marta_test_xx, document.marta_test_yy).click();
      document.marta_test_xx = 0;
      document.marta_test_yy = 0;
    };
  };
